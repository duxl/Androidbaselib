...
public final class SpanUtils {
    ...
    private void updateCharCharSequence() {
        ...
        if (fontSize != -1) {
            mBuilder.setSpan(CharacterStyle.wrap(new AbsoluteSizeSpan(fontSize, fontSizeIsDp)), start, end, flag);
        }
        if (proportion != -1) {
            mBuilder.setSpan(CharacterStyle.wrap(new RelativeSizeSpan(proportion)), start, end, flag);
        }
        if (xProportion != -1) {
            mBuilder.setSpan(CharacterStyle.wrap(new ScaleXSpan(xProportion)), start, end, flag);
        }
        if (lineHeight != -1) {
            mBuilder.setSpan(new CustomLineHeightSpan(lineHeight, alignLine), start, end, flag);
        }
        if (isStrikethrough) {
            mBuilder.setSpan(CharacterStyle.wrap(new StrikethroughSpan()), start, end, flag);
        }
        if (isUnderline) {
            mBuilder.setSpan(CharacterStyle.wrap(new UnderlineSpan()), start, end, flag);
        }
        if (isSuperscript) {
            mBuilder.setSpan(CharacterStyle.wrap(new SuperscriptSpan()), start, end, flag);
        }
        if (isSubscript) {
            mBuilder.setSpan(CharacterStyle.wrap(new SubscriptSpan()), start, end, flag);
        }
        if (isBold) {
            mBuilder.setSpan(CharacterStyle.wrap(new StyleSpan(Typeface.BOLD)), start, end, flag);
        }
        if (isItalic) {
            mBuilder.setSpan(CharacterStyle.wrap(new StyleSpan(Typeface.ITALIC)), start, end, flag);
        }
        if (isBoldItalic) {
            mBuilder.setSpan(CharacterStyle.wrap(new StyleSpan(Typeface.BOLD_ITALIC)), start, end, flag);
        }
        if (fontFamily != null) {
            mBuilder.setSpan(CharacterStyle.wrap(new TypefaceSpan(fontFamily)), start, end, flag);
        }
        if (typeface != null) {
            mBuilder.setSpan(CharacterStyle.wrap(new CustomTypefaceSpan(typeface)), start, end, flag);
        }
        if (alignment != null) {
        ...
        if (shadowRadius != -1) {
            mBuilder.setSpan(CharacterStyle.wrap(
        ...
        if (spans != null) {
            for (Object span : spans) {
                mBuilder.setSpan(CharacterStyle.wrap((CharacterStyle) span), start, end, flag);
            }
        }
        if (backgroundColor != COLOR_DEFAULT) {
            mBuilder.setSpan(CharacterStyle.wrap(new BackgroundColorSpan(backgroundColor)), start, end, flag);
        }
        if (foregroundColor != COLOR_DEFAULT) {
            mBuilder.setSpan(CharacterStyle.wrap(new ForegroundColorSpan(foregroundColor)), start, end, flag);
        }
    }
